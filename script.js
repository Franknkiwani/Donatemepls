...
window.openWalletModal = async () => {
    ...
    el.classList.remove('hidden');
    if(window.toggleScrollLock) toggleScrollLock(true);
    ...
    try {
        ...
        // 2. Update Dual Balances based on your API fields
        const totalTokens = Number(data.tokens || 0);
        const earned = Number(data.totalEarned || 0);
        // Purchased is whatever is left over
...
window.closeWalletModal = () => {
    ...
    if(el) {
        ...
        if(window.toggleScrollLock) toggleScrollLock(false);
    }
};

// --- WITHDRAW LOGIC (Connects to your Vercel API) ---

window.triggerWithdraw = async () => {
    const user = auth.currentUser;
    const earnedVal = parseInt(document.getElementById('wallet-earned-bal').innerText);
    ...
    try {
        ...
        if(result.success) {
            ...
            window.openWalletModal(); // Refresh balances
        } else {
...
window.switchWalletTab = async (tab) => {
    ...
    if (tab === 'received') {
        try {
            ...
            // Note: Since your API currently lacks 'toUid' in the donation log, 
            // we filter by 'toName' matching current username as a temporary fallback.
            // RECOMMENDED: Update Donate API to include 'toUid'
            const userSnap = await get(ref(db, `users/${user.uid}`));
...
const renderWalletHistoryItem = async (tx, container) => {
    // If your API doesn't save fromUid yet, we try to find it via fromName or skip
    ...
    item.className = "flex items-center justify-between p-4 bg-white/5 border border-white/5 rounded-[1.5rem] mb-2 animate-in fade-in slide-in-from-bottom-2";
    
    item.innerHTML = `
        <div class="flex items-center gap-3">
            <div class="relative">
                <img src="${s.avatar || (window.presets ? presets[0] : '')}" class="w-10 h-10 rounded-full object-cover border border-white/10 shadow-lg">
                ${isPrem ? `
                    <div class="absolute -right-1 -bottom-1 bg-blue-500 rounded-full p-0.5 border-2 border-[#0a0a0a]">
                        ${typeof getVerifiedBadge === 'function' ? getVerifiedBadge() : ''}
                    </div>
                ` : ''}
            </div>
            <div>
                <p class="text-[10px] font-black text-white uppercase italic tracking-tight">@${s.username || 'User'}</p>
                <p class="text-[8px] text-zinc-500 font-bold uppercase">
                    ${typeof getRelativeTime === 'function' ? getRelativeTime(tx.timestamp) : 'Recently'}
                </p>
            </div>
        </div>
        <div class="flex flex-col items-end gap-2">
            <span class="text-xs font-black text-emerald-400">+$${tx.amount}</span>
            <button onclick="window.closeWalletModal(); if(window.handleTip) handleTip('${tx.fromUid}', '${s.username}', '${s.avatar}')" 
